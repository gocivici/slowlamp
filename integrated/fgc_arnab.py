...
storage_file = open(f"{filename_time}_fgc_arnab.txt", "w")
...
def get_colors_from_difference(current_img, previous_img, num_colors, separate_vibrant = True):
    ...
    if len(imgNoB.flatten()) <= num_colors * kmeans_lab_medoids_spread.OVERSAMPLE_FACTOR*3: 
    ...
    DominantColors, label_counts_sorted = kmeans_lab_medoids_spread.extract_palette(imgNoB, k=num_colors)
...
def dominantColor(waitTime):
    # time.sleep(waitTime)
    global previous_img, stored_traces, canvas
