...
def get_colors_from_difference(current_img, previous_img, num_colors, separate_vibrant = True):
    ...
    k = num_colors # number of clusters
    ...
    if not separate_vibrant:
        return diff_preview, DominantColors, label_counts_sorted
    ...
    return diff_preview, vibrant_color, vr_count, other_colors, other_counts
...
def display_diagnostic_at_main(canvas, prev_preview, curr_preview, diff_preview, vibrant_color, vr_count, supplemental_colors, supplemental_counts):
    ...
    # display some stuff
        ...
        feed_preview_x = 0
        for i, (count, center) in enumerate(zip(supplemental_counts, supplemental_colors)):
            print("Supplemental (rgb) ", i, center)
        ...
        print("Vibrant (rgb) ", vibrant_color)
        bgr_color = [vibrant_color[2], vibrant_color[1], vibrant_color[0]]
        y_min = feed_preview_y + 4*color_swatch_size
        canvas[y_min:y_min+color_swatch_size, feed_preview_x:feed_preview_x+color_swatch_size] = bgr_color
        canvas = cv2.putText(canvas, f'count {vr_count}', (feed_preview_x+color_swatch_size, y_min+10), font, font_scale, text_color, thickness)
        ...
        largest_color = supplemental_colors[0]
        lg_count = supplemental_counts[0]
...
def display_diagnostic_at_fast_stream(canvas, frame, new_colors, candidate_color = None):
    ...
    if candidate_color is not None:
        canvas[feed_preview_y:feed_preview_y*2, feed_preview_x:feed_preview_x*2, ::-1] = candidate_color[:-1]
    ...
    for ci in range(len(new_colors)): #lab
        ...
        y_min = feed_preview_y*2 + ci*color_swatch_size
    ...
    for i, track in enumerate(fast_tracks):
        ...
        y_min = feed_preview_y*2 + i*color_swatch_size
        canvas[y_min:y_min+color_swatch_size, feed_preview_x:feed_preview_x+color_swatch_size] = bgr_color
...
def dominantColor(waitTime):
    ...
    if waitTime > 0:
        ...
        if num_cycles > 0:
            ...
            for fast_cycle_index in range(num_cycles):
                ...
                new_colors, label_counts = helper_classes.extract_colors_kmeans(curr_between_img, 5, color_mode="lab")
    ...
    time_elapsed = time.time() - start_time_seconds  
    return (supplemental_colors[0], supplemental_counts[0]), (vibrant_color, vr_count), time_elapsed
