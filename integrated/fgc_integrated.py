...
if using_pi:
    ...
    if not diyVersion:
        ...
        camera = Picamera2()
        exposure_time = [3000000,500000,62500]

    config = camera.create_still_configuration(main={"format": 'RGB888', "size":  (800,600)}, controls={"AwbEnable":0, "AwbMode": 3}) #DaylightMode=4, indoor=3
...
#-------------------------Backgorund Substraction-----------------------------------
    ...
    record = [tuple(vibrant_color[:-1]), vr_count,
            ...
            tuple(other_colors[2][:-1]), other_counts[2], tuple(other_colors[3][:-1]), other_counts[3], int(time.time() // (day_length * 60))]
    print(record)
    cover.save(*record)
